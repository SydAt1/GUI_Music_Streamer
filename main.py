...
class GUIMusicPlayer:
    def __init__(self):
        ...
        self.playlist_dir = r"D:\projects\Music_Stream\playlists"
    ...
    def load_playlists(self):
        try:
            ...
            playlist_file = os.path.join(self.playlist_dir, 'playlists.json')
            ...
            if event.type == QUIT:
                ...
                if self.playing_playlist:
                    ...
                    if song:
                        ...
                        self.playing_playlist = None
                        self.show_message("No next song available")
                    self.update_now_playing()
                    self.refresh_history()
            elif event.type == MOUSEWHEEL:
        ...
        if btns[0].handle_event(event, pos):  # List All
            ...
            if current_pl:
                ...
                if song:
                    ...
                    self.playing_playlist = current_pl
                    self.update_now_playing()
                    self.show_message(f"Now at first song: {song['title']}")
                    self.refresh_history()
                self.refresh_playlists()
            ...
            if current_pl:
                ...
                if song:
                    ...
                    self.playing_playlist = current_pl
                    self.update_now_playing()
                    self.show_message(f"Now at last song: {song['title']}")
                    self.refresh_history()
                self.refresh_playlists()
        ...
        if btns[0].handle_event(event, pos):  # Play/Pause
            ...
            if self.playing_playlist:
                ...
                if song:
                    ...
                    self.show_message("No previous song available")
            self.update_now_playing()
            self.refresh_history()
    ...
    def draw(self):
        ...
        if self.current_tab == "library":
            ...
            if self.current_duration > 0:
                ...
                # Display time
                current_time = int(pos / 1000)  # Convert milliseconds to seconds
                total_time = int(self.current_duration / 1000)  # Convert milliseconds to seconds
